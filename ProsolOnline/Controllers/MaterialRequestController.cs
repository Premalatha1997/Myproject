...
namespace ProsolOnline.Controllers
{
    public class MaterialRequestController : Controller
    {
        ...
        public JsonResult newRequest()
        {
            ...
            //string plant, string storage, string group, string subgroup, string approver, string source
        ...
        public JsonResult bulk_material_request()
        {
            ...
            var tbl = new DataTable();            tbl.Columns.Add("Plant");            tbl.Columns.Add("Storage Location");            tbl.Columns.Add("Material Type");            tbl.Columns.Add("Industrial Sector");            tbl.Columns.Add("Material Group");            tbl.Columns.Add("Source");            foreach (ItemrequestModel pr1 in mirm)            {                var row = tbl.NewRow();                row["Plant"] = pr1.hplant;                row["Storage Location"] = pr1.hstorage_Location;                row["Material Type"] = pr1.hmaterialtype;                row["Industrial Sector"] = pr1.hIndustrysector;                row["Material Group"] = pr1.hMaterialStrategicGroup;                row["Source"] = pr1.hsource;                tbl.Rows.Add(row);
                ...
                if (files != null)
                {
                    bool mul_resilt = _ItemRequestService.insert_multiplerequest(m_mul_req_values, date1, files);
        ...
        public JsonResult bulkupload_material_request()
        {
            ...
            foreach (Prosol_Request pr2 in m_mul_req_values)
            {
                ...
                {
                    if (pr2.plant == pm.Plantcode && pr2.storage_Location == pm.Code)
                    {
                        slil = 1;
                ...
                { 
                var MatTypeRes = findmaster.Where(x => x.Label == "Material type" && x.Code==pr2.Materialtype).ToList();
                ...
                {
                    pr2.Materialtype = MatTypeRes[0].Title;
                }
                }
                ...
                {
                    var IndSecRes = findmaster.Where(x => x.Label == "Industry sector" && x.Code == pr2.Industrysector).ToList();
                    ...
                    {
                        pr2.Industrysector = IndSecRes[0].Title;
                    }
                }
                ...
                {
                    var MatStrgGrpRes = findmaster.Where(x => x.Label == "Material Strategic Group" && x.Code == pr2.MaterialStrategicGroup).ToList();
                    if (MatStrgGrpRes.Count == 0)
                    {
                        return this.Json("Material Strategic Group " + pr2.MaterialStrategicGroup + " is not exist in the master", JsonRequestBehavior.AllowGet);
                    }
                    else
                    {
                        pr2.MaterialStrategicGroup = MatStrgGrpRes[0].Title;
                    }
                }
                ...
                if (pr2.subGroup != null && pr2.subGroup != "")
                {
                    ...
                    {
                        if (pr2.group == pm.groupCode && pr2.subGroup == pm.title)
                        {
                            sgil = 1;
