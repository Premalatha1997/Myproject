...
(function () {
    ...
    //app.controller('CatalogueController', ['$scope', '$http', '$timeout', '$filter', function ($scope, $http, $timeout, $filter, $window, $location) {
        app.controller('CatalogueController', function ($scope, $http, $timeout, $filter, $window, $location) {
        ...
        $scope.RowClick = function (lst, idx) {
            ...
            $http({
                ...
                if ((response.Review != null && response.Review.Name == usr) && (response.ItemStatus == 2 || response.ItemStatus == 3))
         ...
         //   alert("hai");
            ...
            if (confirm("Are you sure, rework this record?")) {
                ...
                $scope.cgBusyPromises = $http({
                    ...
                    params : {itemcode:$scope.cat.Itemcode , RevRemarks:$scope.cat.RevRemarks}
                }).success(function (data, status, headers, config) {
                    if (data.success === false) {
        ...
        $scope.RejectData = function  () {
           ...
           if ($scope.cat.RevRemarks == undefined || $scope.cat.RevRemarks == null) {
